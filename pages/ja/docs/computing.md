...
1. `/dimbula use`で利用したいモバイルを選択
2. 利用できる空きモバイルを確保し、リモートで利用するためのURLをチャンネルにポスト
3. ブラウザでURLを表示し、映像再生ボタンで利用開始
4. 利用を終了するときは、ブラウザを閉じ、`/dimbula quit`でモバイルを解放する
...
ボタン押下すると、プロンプトが表示されますので、スクリーンショットのファイル名を拡張子なしで入力してください。
...
記録した操作履歴に加えて、実機モバイルの情報とOSの言語、各操作間の待ち時間が設定されます。OSの言語は、現在表示している言語がデフォルトでチェックされていますが、E2Eテストでテストしたい言語を複数選択できるので、ここで他の言語をチェックください。

### YAMLファイルアップロードボタン
ダウンロードしたYAMLファイルをアップロードし、履歴として使うことができます。
